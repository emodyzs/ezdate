...
def d_lookup(strIn,d_samples,tupleout=False,bWholesamples=True):
    ...
    d_samples:   {[keresőszó]:[value],...}         A keresőszó string, a value lehet szám is vagy akár egy sub dictironary
       teljes-egyezéses minta: pont a keresőszó végén
    tupleout: True esetén return (key,value)
    ...
    if bWholesamples:
        result=d_samples.get(keresoszo + '.')
        if result: 
            if tupleout: return keresoszo,result
            else: return result

    # Keresés hosszrövidítésekel (a leghosszabb keresőminta érvényesül)
    while len(keresoszo)>1:
        #print(str(type(d_samples)))
        result=d_samples.get(keresoszo)
        if result: 
            if tupleout: return keresoszo,result
            else: return result
        keresoszo=keresoszo[:-1]
...
def d_annotate(strIn,lookup,bNum=False):    # -> pattern,invalues,outvalues
    ...
    strIn:  általában több szavas, lower és trim előkészítéssel  (használható pl a clean() helper)
    lookup:   {'[startwords]':'[entity],[value]', ...}
        - [startwords]: több szavas is lehet | határolással.  Példa:  'múlt|elmúlt|előző|legutóbbi'
        ...
        - [entity]:  entity-név (pl. "évszak").  Előfordulhat, hogy ugyanaz az entity többször is előfordul a mondatban
                Ugyanaz az entitás-név több startwords-höz is megadható.  Tetszőleges számú entitásnév lehet a d_regex-ben.
                "none" entitást kell megadni, ha csak szaványosításról illetve szinonimák összevonásáról van szó
                  Példa:  "korábbi|előző":none,korábbi.     Ebben az esetben csere a szabványosított értékre, a d_values-be nem kerül bele
                "stopword" entitás-névvel kell felsorolni az elhagyandó szavakat (pl. névelők)
        - [value]:  ez az érték kerül be a d_values-be (az adott entity névvel). Szabványosított / kivonatolt érték
                Ha nincs megadva, akkor megegyezik a startword-del
                Lehet szám, dátum, string is
    ...
    return   pattern,invalues,outvalues
                pattern: "[entity1] egyébszó [entity2]"
                invalues:  az annotált részek eredeti értéke  (a fenti példához 2-elemű lista tartozik)
                outvalues:  az annotált részek kimeneti értéke  (a fenti példához 2-elemű lista tartozik)
    '''

    invalues=[]
    outvalues=[]

    # key-felsorolások kibontása
    if not lookup.get('d_compiled'):
        dict_compiled={'d_compiled':True}
        for key,value in lookup.items():
            subkeys=key.split('|')
            for subkey in subkeys: 
                if subkey: dict_compiled[subkey]=value
        lookup=dict_compiled
    

    # A VÁLTOZAT:  szavanként keresés a mintákat tartalmazó dict-ben  (like: karakter-elhagyások a végéről)
    # probléma:  lehetnek teljes szavas keresőminták is
    #  megoldás:  a teljes szavas mintákat el kell különíteni
    #     d_like   d_wholeword
    # VAGY listák:
    #   a dict előfeldolgozása keretében külön rekordba sorolandók
    #   "korábbi*|előző*:none,korábbi
    #     korábbi*:none,korábbi
    #     előző*:none,korábbi
    ...
    strL=strL.lower()
    # str=str.translate(str.maketrans('íóőúű','ioöuü'))
    ...
    for word in words:
        ...
        if bNum:
            if not (word=='hét' and len(wordsout)>0):   # a "hét" szó helyébe ne írjon számot, ha nem az első szó (összetéveszthető a "hét" időhatározóval)
